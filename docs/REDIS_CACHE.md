...
| **分布式锁**     | 预热互斥、按队伍加队互斥   | `infra/lock` + `cache_task` / `team.Service` | 极少 key、短 TTL       |
...
实现：`infra/lock` → `port.Locker.RunWithLock`（`SET NX EX` + Lua 按 token 释放）。

| Key | TTL | 调用方 | 失败语义 |
| --- | --- | ------ | -------- |
| `lock:cron:warmup_match_users` | 10 分钟 | `user.WarmUpMatchUsers` | `ErrLockFailed` → 其它节点跳过预热 |
| `yupao:join_team:{teamID}` | 10 秒 | `team.Service.Join` | `ErrLockFailed` → 业务「系统繁忙」 |

锁 key 极少、TTL 短，对内存可忽略。预热锁避免并行放大写压力；加队锁 **按队伍** 串行化「人数校验 + 写成员」，不同队伍互不阻塞（同一用户跨多队并发撞「最多 5 队」概率低，暂不按 user 加锁）。

相关日志：预热见 `warmup.*`（`purpose=job`）；加队抢锁失败见 `team.join_busy`（`purpose=biz`，Warn）。结构化约定见 [`internal/pkg/logger/README.md`](../internal/pkg/logger/README.md)。

队伍 **退出 / 解散 / 队长移交** 使用数据库 `team` 行锁（`FOR UPDATE`），与本节 Join 的 Redis 锁不是同一把锁；边界与演进见 [`internal/module/team/TX_LOCK.md`](../internal/module/team/TX_LOCK.md)。
...
| `yupao:join_team:{teamID}`     | 加队分布式锁（按队伍） |
...
| `internal/module/team/service.go`    | 加队锁 `yupao:join_team:{teamID}`   |
| `internal/module/user/CACHE.md`      | 匹配缓存细节与架构图                |
| `internal/pkg/logger/README.md`      | 缓存/预热相关日志 event             |
